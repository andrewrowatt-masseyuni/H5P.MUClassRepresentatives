/* To Do:
Ad for Class Reps?
*/
var H5P = H5P || {};
 
H5P.MUClassRepresentatives = (function ($) {
  /**
   * Constructor function.
   */
  function C(options, id) {
    this.options = options;
    this.id = id;
  };
 
  /**
   * Attach function called by H5P framework to insert H5P content into
   * page
   *
   * @param {jQuery} $container
   */
  C.prototype.attach = function ($container) {
    // container.  Allows for styling later.
    $container.addClass("h5p-reps");
	//console.log("layout");
	//console.log(this.options.layout);
	
	if(this.options.layout == '2') {
		$container.addClass("h5p-reps-twocol");
	} else if (this.options.layout == '3') {
		$container.addClass("h5p-reps-threecol");
	} else if (this.options.layout == '4') {
		$container.addClass("h5p-reps-fourcol");
	} else if((this.options.layout === undefined || this.options.layout == 'auto') && this.options.reps.length<=4) {
		$container.addClass("h5p-reps-twocol");
	}
	else {
		$container.addClass("h5p-reps-threecol");
	}

	var html = '';

	for(rep of this.options.reps) {
		html = '<div class="rep">';
		
		if (rep.image && rep.image.path) {
			html += '<img alt="' + rep.name + '" class="profile-image" src="' + H5P.getPath(rep.image.path, this.id) + '">';
		} else {
			html += '<img alt="" class="profile-image" src="data:image/jpeg;base64,">';
		}
		
		html += '<div class="rep-item name-text">' + rep.name + '</div>';
		if(rep.role) html += '<div class="rep-item role-text">' + rep.role + '</div>';
		html += '<div class="rep-item contact-text">' + rep.contact + '</div>'
		if(rep.about) html += '<div class="rep-item about-text">' + rep.about + '</div>'
		html += '</div>';

		$container.append(html);
	}
  };
 
  return C;
})(H5P.jQuery);